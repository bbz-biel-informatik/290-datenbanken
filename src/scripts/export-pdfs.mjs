...
async function main() {
    ...
    try {
        ...
        console.error(`Slides directory not found: ${slidesDir}. Did you run 'npm run build' first?`);
        process.exit(1);
    }

    const htmlFiles = await collectHtmlFiles(slidesDir);
    if (htmlFiles.length === 0) {
        console.log(`No .html files found in ${slidesDir}. Skipping PDF generation.`);
        return;
    }
