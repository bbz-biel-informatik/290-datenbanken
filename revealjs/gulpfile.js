...
const root = path.resolve(__dirname, argv.root || "..");
...
gulp.task(
  ...
  gulp.series(async () => {
    ...
    if (fs.existsSync(path.resolve(__dirname, "../slides")))
      dirs.push("../slides/**");

    return gulp
      .src(dirs, { base: path.resolve(__dirname, ".."), encoding: false })
...
gulp.task("reload", () =>
  gulp.src([path.join(root, "**/*.html")]).pipe(connect.reload())
);
