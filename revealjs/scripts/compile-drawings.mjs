...
const revealDir = path.resolve(scriptDir, '..');
const repoRoot = path.resolve(revealDir, '..');
    ...
    for (const inputPath of files) {
        ...
        if (shouldCompile) {
            ...
            try {
                ...
                // We use paths relative to revealjs dir where the command is executed
                const relInput = path.relative(revealDir, inputPath);
                const relOutput = path.relative(revealDir, outputPath);
                
                await execAsync(`npx excalidraw-to-svg "${relInput}" "${relOutput}"`, { cwd: revealDir });
            } catch (error) {
